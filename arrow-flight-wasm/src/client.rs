...
pub struct FlightClient {
    ...
    /// The inner client
    inner: FlightServiceClient<Client>,
}

impl FlightClient {
    /// Creates a client client with the provided [`Channel`]
    pub fn new(channel: Client) -> Self {
    ...
    pub fn new_from_inner(inner: FlightServiceClient<Client>) -> Self {
        Self {
    ...
    /// [`FlightServiceClient`]
    pub fn inner(&self) -> &FlightServiceClient<Client> {
        &self.inner
    }

    /// Return a mutable reference to the underlying tonic
    /// [`FlightServiceClient`]
    pub fn inner_mut(&mut self) -> &mut FlightServiceClient<Client> {
        &mut self.inner
    }

    /// Consume this client and return the underlying tonic
    /// [`FlightServiceClient`]
    pub fn into_inner(self) -> FlightServiceClient<Client> {
        self.inner
    }
